...
void SJF(PCBL *L_pointer)
{
   ...
   Lp=SortSJF(L_pointer);
    p=Lp->next;
   while(p!=NULL)
   {
   		if(p->data.ID!=Lp->next->data.ID) 
		{ 
...
//使用 一个局部变量FinishTime来保存上一作业的完成时间，然后判断临节点，
//当作业的到达时间小于上一作业的完成时间，产生了交集，这时即使作业已经到了，还得上一作业完成才轮到
//当前作业，所以作业完成时间=上一作业完成时间+本作业的服务时间 
